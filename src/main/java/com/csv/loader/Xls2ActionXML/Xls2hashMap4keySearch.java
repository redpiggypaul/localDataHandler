 ...
 */
public class Xls2hashMap4keySearch {
    ...
    public static void main(String args[]) throws IOException {
        ...
        try {
            ...
            String actionXMLtemplateName = autoToolPath + "src/main/java/REXSH/REXAUTO/Component/AppAction/action2Page_Template.java";
            List<globalActionKeyEntity> actionLinelist = readExcel4hashMap(userDocuPath + "csvloader//src//main//java//com//csv//action.xls", 0, actionXMLtemplateName);      //todo change for windows
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static List<globalActionKeyEntity> readExcel4hashMap(String filePath, int rowIndex, String actionXMLtemplateName) throws Exception {
        List<globalActionKeyEntity> globalActionNameList = new ArrayList<globalActionKeyEntity>();
        List<actionKeyEntity> localActionNameList = new ArrayList<actionKeyEntity>();
        InputStream inputStream = null;
        String autoToolPath = readProperity.getProValue("templateFilePath"); // templateFilePath = //Users//appledev131//Documents//REXAUTO4//
        try {
            inputStream = new FileInputStream(filePath);
            ...
            for (int i = 0; i < sheetSize; i++) {
                ...
                for (globalActionKeyEntity tEx : globalActionNameList) {
                    gMap.put(tEx.getOpeName(), tEx.getGroupName());
                }
                // 1.获取sheet表
                HSSFSheet sheet = hssfWorkbook.getSheetAt(i);
                ...
                if (sheet.getSheetName().equalsIgnoreCase("actionSample")) {
                    ...
                    // 2.解析sheet表,如果没有指定从第几行开始读取数据,则默认从第二行起，对excel内容进行解析
                    HashMap<String, String> localActionLineList = new HashMap<String, String>();
                    ...
                    if (!sheet.getSheetName().equalsIgnoreCase("")) {
                        ...
                        for (int j = rowIndex; j <= lastRowNum; j++) {
                            actionKeyEntity singleTcEntry = new actionKeyEntity();
                            String nameP1 = null;
                            String nameP2 = null;
                            //   rowList = new ArrayList<String>();
                            HSSFRow hssfRow = sheet.getRow(j);
                            ...
                            if (hssfRow != null) {
                                for (int cellIndex = 0; cellIndex < lastCellNum; cellIndex++) {
                                    ...
                                    if (cellIndex == index4OpeName) {
                                        nameP1 = new String(cell.getStringCellValue());
                                        singleTcEntry.setOpeName(cell.getStringCellValue());
                                        //   singleKeyEntry.setGroupName(sheet.getSheetName());
                                        //   singleKeyEntry.setOpeName(cell.getStringCellValue());

                                    }
                                    if (cellIndex == index4Step) {
                                        singleTcEntry.setStep(cell.getStringCellValue());
                                        nameP2 = new String(cell.getStringCellValue());
                            ...
                            if (localActionLineList.get(nameP1 + nameP2) == null) {
                                localActionLineList.put(nameP1 + nameP2, "");
                                localActionNameList.add(singleTcEntry);
                            } else {
                                throw new Exception("Duplicated item in " + sheet.getSheetName() + " :: " + nameP1 + " :: " + nameP2);
                            }
                        }
                        ArrayList<String> opeName = new ArrayList<String>();
                        // localActionNameList.add(singleTcEntry);
                        for (actionKeyEntity tempEn : localActionNameList) {
                            opeName.add(tempEn.getOpeName());

                            //  globalActionNameList.add(singleKeyEntry);
                        }
                        opeName = removeDuplicate(opeName);


                        localActionLineList.clear();
                        localActionNameList.clear();
                        for (String tt : opeName) {
                            if (gMap.get(tt) == null) {
                                globalActionKeyEntity singleKeyEntry = new globalActionKeyEntity();
                                singleKeyEntry.setGroupName(sheet.getSheetName());
                                singleKeyEntry.setOpeName(tt);
                                globalActionNameList.add(singleKeyEntry);
                            } else {
                                throw new Exception("Duplicated key word : " + tt + " in " + sheet.getSheetName() + " AND " + gMap.get(tt));
                            }
                        }
                    }
            ...
            File targetFile = new File(autoToolPath + "src/main/java/REXSH/REXAUTO/Component/AppAction/action2PageOperationXML.java");
            if (globalActionNameList.size() == 0) {
                throw new FileNotFoundException("The Action Map java file is missing!");
            }
            // 读取文件
            ...
            if (totalNo == 22) {
                while ((singleBufferWriterLine = br.readLine()) != null) {
                    //action2Page_Template
                    if(ind==9){
                        String rowContent = getLineVarFile(fileName, ind);
                        rowContent = new String(rowContent.replaceFirst("action2Page_Template", "action2PageOperationXML"));
                        ...
                        for (globalActionKeyEntity theSingle : globalActionNameList) {
                            readLineVarFile(fileName, ind);
                            if (theSingle.getOpeName() != null) {

                                rowContent = new String(rowContent.replaceFirst("and_action_key", theSingle.getOpeName()));
                                rowContent = new String(rowContent.replaceFirst("Action_and_file.xml", "Action_"+theSingle.getGroupName()+".xml"));
                                bufferWriter.write(rowContent);//输出字符串
                                bufferWriter.newLine();//换行
                                bufferWriter.flush();
                            }
